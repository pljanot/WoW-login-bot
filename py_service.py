...
realmsScreenWords = ['english', 'realm', 'gehennas', 'firemaw', 'dreadmist', 'noggenfogger', 'full']
...
loggedInSomewhereElse = ['your', 'account', 'has', 'logged', 'in', 'blz519000060']
...
def getSceneText():
    ...
    minX, maxX, minY, maxY = int(cntX - 0.5 * cntX), int(cntX + 0.5 * cntX), int(cntY - 0.75 * cntY), int(
        cntY + 0.9 * cntY)
...
def idler():
    ...
    Popen([WOW_PATH])
    pyautogui.moveTo(int(pyautogui.size()[0] / 2) + int(pyautogui.size()[0] / 6), int(pyautogui.size()[1] / 2),
                     duration=fakeDelay(2000, 4000))
    last = -1
    unknownCnt = 0
    while (True):
        ...
        if state <= last and not (state == 3 or state == 4 or state == 5 or state == 6):
            state = -1
        else:
            last = state
            unknownCnt = 0
        ...
        if state == 0:
            ...
            if unknownCnt >= 10:
                ...
                unknownCnt = 0
                randomWait(0.5, 1)
                pyautogui.moveTo(int(pyautogui.size()[0] / 2) + int(pyautogui.size()[0] / 6),
                                 int(pyautogui.size()[1] / 2), duration=fakeDelay(2000, 4000))
                randomWait(0.5, 1)
            randomWait(1, 3)
